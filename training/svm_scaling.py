...
scaling_process_11 = subprocess.run(['svm-scale.exe', '-s',
                                     'range_position_lhand',
                                     'position_lhand_unscaled.train',
                                     '>',
                                     'position_lhand_scaled.train'],
                                    shell=True,  # Necessary for the > operator
                                    stdout=subprocess.PIPE)

scaling_process_12 = subprocess.run(['svm-scale.exe', '-s',
                                     'range_position_lhand_new_features',
                                     'position_lhand_unscaled_new_features.train',
                                     '>',
                                     'position_lhand_scaled_new_features.train'],
                                    shell=True,  # Necessary for the > operator
                                    stdout=subprocess.PIPE)

scaling_process_13 = subprocess.run(['svm-scale.exe', '-s',
                                     'range_position_lhand_combined',
                                     'position_lhand_unscaled_combined.train',
                                     '>',
                                     'position_lhand_scaled_combined.train'],
                                    shell=True,  # Necessary for the > operator
                                    stdout=subprocess.PIPE)

scaling_process_14 = subprocess.run(['svm-scale.exe', '-s',
                                     'range_position_rhand',
                                     'position_rhand_unscaled.train',
                                     '>',
                                     'position_rhand_scaled.train'],
                                    shell=True,  # Necessary for the > operator
                                    stdout=subprocess.PIPE)

scaling_process_15 = subprocess.run(['svm-scale.exe', '-s',
                                     'range_position_rhand_new_features',
                                     'position_rhand_unscaled_new_features.train',
                                     '>',
                                     'position_rhand_scaled_new_features.train'],
                                    shell=True,  # Necessary for the > operator
                                    stdout=subprocess.PIPE)

scaling_process_16 = subprocess.run(['svm-scale.exe', '-s',
                                     'range_position_rhand_combined',
                                     'position_rhand_unscaled_combined.train',
                                     '>',
                                     'position_rhand_scaled_combined.train'],
                                    shell=True,  # Necessary for the > operator
                                    stdout=subprocess.PIPE)